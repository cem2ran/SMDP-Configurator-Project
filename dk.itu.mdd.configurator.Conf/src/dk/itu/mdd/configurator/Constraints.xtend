...
public class Constraints {
	...
	def static dispatch Boolean constraint(Binary exp) {
		...
		if ((rexp as ConstrainImpl).featureReference instanceof Feature &&
			(lexp as ConstrainImpl).featureReference instanceof Feature) {
		...
		switch (BinaryOperator.getByName(exp.operator.getName()).getName.toString) {
			case "lessThan":
				if ((lexp as ConstrainImpl).constrainValue instanceof modelMDD2.Number &&
					(rexp as ConstrainImpl).constrainValue instanceof modelMDD2.Number) {
					...
					if (!checkAttributesType((lexp as ConstrainImpl).constrainValue,
						(((rexp as ConstrainImpl).featureReference as Grouped) as Feature).attribute)) {
						new Boolean(false);
					} else {
						val lval = ((lexp as ConstrainImpl).constrainValue as modelMDD2.Number).value;
					...
					if (!checkAttributesType((rexp as ConstrainImpl).constrainValue,
						(((lexp as ConstrainImpl).featureReference as Grouped) as Feature).attribute)) {
						new Boolean(false);
					} else {
						val rval = ((rexp as ConstrainImpl).constrainValue as modelMDD2.Number).value;
	...
	def static checkRangeValidity(Grouped it) {
		...
		//return attribute.filter[attribute.forall[a  | a instanceof RangeImpl ]].forall[a | (a as RangeImpl).upper > (a as RangeImpl).lower];
